...
# Summary Information based on the number of medals

summary_info <- list()
summary_info$num_observations <- nrow(olympic_volleyball_medals)
summary_info$medals_min_val <- cummulative_medals_all_data %>%
  filter(cummulative_medals == min(cummulative_medals,na.rm = T)) %>%
  select(cummulative_medals)

summary_info$medals_max_val <- cummulative_medals_all_data %>%
  filter(cummulative_medals == max(cummulative_medals,na.rm = T)) %>%
  select(cummulative_medals)

summary_info$medals_mean_val <- cummulative_medals_all_data %>%
   summarise(mean_val = mean(cummulative_medals))

summary_info$medals_median_val <- cummulative_medals_all_data %>%
  summarise(median_val = median(cummulative_medals))

summary_info$medals_sd <- cummulative_medals_all_data %>%
  summarise(sd_val = sd(cummulative_medals))
